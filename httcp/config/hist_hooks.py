            ...
            for njet in njets:
                for xgb in xgb_nodes:
                    for cat_id in category_ids:
                        ...
                        # CASE OF AB CLOSURE PLOTS
                        if type_extrapolation == "AB":
                            if cat_inst.has_tag({"ss", "iso1", njet, xgb, dm}, mode=all) and not cat_inst.has_tag("noniso2"): # cat A 
                                qcd_groups[f"bdt_{xgb}_dm_{dm}_njet_{njet}"].os_iso = cat_inst
                            elif cat_inst.has_tag({"ss", "noniso1", njet, xgb, dm}, mode=all) and not cat_inst.has_tag("noniso2"): # cat B
                                qcd_groups[f"bdt_{xgb}_dm_{dm}_njet_{njet}"].os_noniso = cat_inst
                                
                        # CASE OF CD CONTROL PLOTS
                        elif type_extrapolation == "CD":
                            if cat_inst.has_tag({"os", "iso1", njet, xgb, dm}, mode=all) and not cat_inst.has_tag("noniso2"): # cat D
                                qcd_groups[f"bdt_{xgb}_dm_{dm}_njet_{njet}"].os_iso = cat_inst
                            elif cat_inst.has_tag({"os", "noniso1", njet, xgb, dm}, mode=all) and not cat_inst.has_tag("noniso2"): # cat C
                                qcd_groups[f"bdt_{xgb}_dm_{dm}_njet_{njet}"].os_noniso = cat_inst
                    
                        # CASE OF C0D0 CONTROL PLOTS
                        elif type_extrapolation == "C0D0":
                            if cat_inst.has_tag({"os", "iso1", njet, xgb, dm}, mode=all) and not cat_inst.has_tag("noniso2"): # cat D0
                                qcd_groups[f"bdt_{xgb}_dm_{dm}_njet_{njet}"].os_iso = cat_inst
                            elif cat_inst.has_tag({"os", "noniso1", njet, xgb, dm}, mode=all) and not cat_inst.has_tag("noniso2"): # cat C0
                                qcd_groups[f"bdt_{xgb}_dm_{dm}_njet_{njet}"].os_noniso = cat_inst   
